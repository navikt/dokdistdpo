...
import static no.nav.dokdistdpo.consumer.dpo.dokumentpakke.StandardBusinessDocumentMapper.ARKIVMELDING_TYPE_VERSION;
import static no.nav.dokdistdpo.consumer.dpo.dokumentpakke.StandardBusinessDocumentMapper.DOKUMENTIDENTIFICATION_TYPE_ARKIVMELDING;
import static no.nav.dokdistdpo.consumer.dpo.dokumentpakke.StandardBusinessDocumentMapper.DOKUMENTIDENTIFICATION_TYPE_AVTALTMELDING;
import static no.nav.dokdistdpo.consumer.dpo.dokumentpakke.StandardBusinessDocumentMapper.MESSAGE_CHANNEL_INSTANCE_IDENTIFIER;
import static no.nav.dokdistdpo.consumer.dpo.dokumentpakke.StandardBusinessDocumentMapper.SCOPE_MESSAGECHANELL_IDENTIFIER;
import static no.nav.dokdistdpo.consumer.dpo.dokumentpakke.StandardBusinessDocumentMapper.TYPE_VERSION;
...
class StandardBusinessDocumentMapperTest {
	...
	void shouldMapDpoAvtaltmelding() {
		...
		StandardBusinessDocument sbd = mapper.mapDpoMeldingEnvelope(createNavDokumentpakke(DPO_AVTALEMELDING), ARKIVMELDING_XML);

		StandardBusinessDocumentHeader sbdh = sbd.getStandardBusinessDocumentHeader();

		assertThat(sbdh.getHeaderVersion()).isEqualTo(StandardBusinessDocumentMapper.HEADER_VERSION);
	...
	void shouldMapDpoArkivmelding() {
		...
		StandardBusinessDocument sbd = mapper.mapDpoMeldingEnvelope(createNavDokumentpakke(DPO_ARKIVMELDING), ARKIVMELDING_XML);
