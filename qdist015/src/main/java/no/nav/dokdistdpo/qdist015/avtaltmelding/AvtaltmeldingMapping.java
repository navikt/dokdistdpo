...
import no.arkivverket.standarder.noark5.arkivmelding.avtalt.EnhetsidentifikatorType;
import no.arkivverket.standarder.noark5.arkivmelding.avtalt.FoedselsnummerType;
...
import no.arkivverket.standarder.noark5.arkivmelding.avtalt.ObjectFactory;
...
import static no.nav.dokdistdpo.qdist015.Qdist015Constant.AVTALTMELDING_NAMESPACE;
...
public class AvtaltmeldingMapping {

	private final EregConsumer eregConsumer;
	private final PdlConsumer pdlConsumer;
	private final SafJournalpostService<CompactSafJournalpost> compactSafJournalpostService;

	public AvtaltmeldingMapping(@Qualifier("CompactSafJournalpostQueryService") SafJournalpostService<CompactSafJournalpost> compactSafJournalpostService,
								EregConsumer eregConsumer, PdlConsumer pdlConsumer) {
	...
	public JAXBElement<Arkivmelding> createAvtaltmelding(SafJournalpost safJournalpost, String bestillingsId) {
		...
		arkivmelding.useSystem(APP_NAME)
				.useMeldingId(bestillingsId).useTidspunkt(datoAvtaltmeldingOpprettet)
	...
	private Saksmappe createAndPopulateSaksmappe(SafJournalpost safJournalpost,
		...
		Journalpost journalpost = mapJournalpost(safJournalpost, datoAvtaltmeldingOpprettet, objectFactory);
		...
		saksmappe.useTittel(safJournalpost.temanavn())
				...
				.useVirksomhetsspesifikkeMetadata(mapNavMappe(safJournalpost.sak().arkivsaksnummer()))
	...
	private Dokumentobjekt createAndPopulateDokumentObjekt(SafJournalpost safJournalpost,
		...
		return dokumentobjekt.useVersjonsnummer(ONE)
				...
				.useReferanseDokumentfil(getReferanseDokumentfil(safJournalpost.journalpostId(), dokumentInfo, avtaltFilformat.getFormat()));
	}
	...
	private String getReferanseDokumentfil(String journalpostId, SafJournalpost.DokumentInfo dokumentInfo, String filformat) {
		return format(REFERANSE_DOKUMENTFIL_FORMAT, journalpostId, dokumentInfo.dokumentInfoId(), getVariantformat(dokumentInfo), filformat);
	}
	...
	private Korrespondansepart createAndPopulateKorrespondansepartMottaker(SafJournalpost safJournalpost, ObjectFactory objectFactory) {
		...
		return mottaker.useKorrespondanseparttype(MOTTAKER)
				...
				.useKorrespondansepartNavn(getSakspartNavnDAP(safJournalpost));
	...
	private Part createAndPopulatePartDAP(SafJournalpost safJournalpost, ObjectFactory objectFactory) {
		...
		if (isBrukerTypeOrgNr(safJournalpost.bruker())) {
			...
			partDAP.setFoedselsnummer(new FoedselsnummerType()
					.useFoedselsnummer(getFoedselsnummer(safJournalpost)));
		}
		return partDAP;
	}
